...
export default {
	async fetch(request, env) {
		try {
			...
			// 如果 env.URL 存在，则添加到数组，否则使用默认值
			if (env.URL) {
				try {
					urls = await ADD(env.URL);
				} catch (error) {
					console.error('处理 URL 环境变量时出错:', error);
					// 保持使用默认的 urls 数组
				}
			}
...
async function ADD(envadd) {
	try {
		if (!envadd) return urls; // 如果输入为空，返回默认数组
		
		// 将制表符、双引号、单引号和换行符都替换为逗号
		var addtext = envadd.replace(/[	|"'\r\n]+/g, ',').replace(/,+/g, ',');
		
		// 删除开头和结尾的逗号
		addtext = addtext.trim().replace(/^,+|,+$/g, '');
		
		// 如果处理后的文本为空，返回默认数组
		if (!addtext) return urls;
		
		// 使用逗号分割字符串，得到地址数组
		const add = addtext.split(',');
		
		// 验证数组内容
		if (!add.length) return urls;
		
		return add;
	} catch (error) {
		console.error('ADD 函数处理出错:', error);
		return urls; // 发生错误时返回默认数组
	}
}
